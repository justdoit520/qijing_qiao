...
def main():
    ...
    result_base = 'E:\lab_related\mycode\qijing_qiao385\\result_newdata'
    model_begin_l = ['CDN_LMT_16','CDN_LMT_4']
    acc_begin_l = [0.9, 0.9]
    ...
    for i, model_folder in enumerate(model_begin_l):
        ...
        if m_len == 1 or model_file[1] != model_path + '\model0.pkl':
            del_path = result_base + '\\' + model_folder + '\\best_train_set_now.csv'
            del_out_path = base_path +  '\del_input_newdata_shuffle.csv'
            length = del_input_4000('E:\lab_related\mycode\qijing_qiao385\data\input_derbin_new.csv', del_path, del_out_path)
            ...
            save_path = 'E:\lab_related\mycode\qijing_qiao385\\result_newdata_2\\' + model_folder  # 存放结果 -------------------------------改了这里----------------------
            mkdir(save_path)
