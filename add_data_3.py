...
meka_classifiers = ['CT','CDN', 'CDT']
weka_classifiers = ['trees.LMT', 'trees.RandomForest']
...
def main():
    ...
    #shuffle('D:\lab_related\my_code\qijing_qiao\data\\train.csv', args)
    ...
    for meka_c in meka_classifiers:
        for weka_c in weka_classifiers:
            ...
            for batch_size in [4,8,16]:
                ...
                accs=[]
                accs.append(acc0)
                for batch in range(3990//batch_size):
                    ...
                    for file in model_file:
                        if file != best_model and file != (model_path+'\model0.pkl'):
                            os.remove(file)
                f_detail.write('\n\n\n')
                f_detail.write('acc change\n')
                for j, acc in enumerate(accs):
                    f_detail.write(str(acc))
                    if (j+1) % 15 == 0:
                        f_detail.write('\n')
                    else:
                        f_detail.write(',')
